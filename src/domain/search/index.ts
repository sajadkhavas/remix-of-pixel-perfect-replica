...
const asString = (value: unknown): string | undefined =>
  typeof value === "string"
    ...
    : Array.isArray(value) && typeof value[0] === "string"
      ? value[0]
      : undefined;
const uniqueSorted = <T extends string | number>(values: readonly T[]): readonly T[] =>
  [...new Set(values)].sort((a, b) => String(a).localeCompare(String(b), "en"));
...
const numberList = (value: unknown, min: number, max: number): readonly number[] =>
  uniqueSorted(
    (asString(value) ?? "")
      .split(",")
      .map(Number)
...
const enumList = <T extends string>(value: unknown, allowed: readonly T[]): readonly T[] =>
  list(value).filter((item): item is T => (allowed as readonly string[]).includes(item));
...
export function parseDiscoverySearch(
  ...
  const category =
    categoryCandidate && SLUG.test(categoryCandidate) ? categoryCandidate : undefined;
...
export function getDiscoverySeoDecision(
  ...
  if (hasFacets)
    return {
      ...
      canonicalSearch: state.page > 1 ? `page=${state.page}` : "",
      reason: "facets",
    };
  if (state.page > 1)
    return { robots: "index,follow", canonicalSearch: `page=${state.page}`, reason: "pagination" };
  if (state.sort !== defaultSort || state.view !== "grid")
    return { robots: "noindex,follow", canonicalSearch: "", reason: "sort-or-view" };
...
export const SORT_DEFINITIONS: readonly SortDefinition[] = [
  {
    value: "relevance",
    labelFa: "مرتبط‌ترین",
    semantics: "Search ranking score; only meaningful with q.",
    requiredData: "search score",
    fallback: "newest",
    indexabilityEffect: "noindex-and-strip-from-canonical",
  },
  {
    value: "newest",
    labelFa: "جدیدترین",
    semantics: "Descending releasedAt, then publishedAt, then stable ID.",
  ...
  },
  {
    value: "price-asc",
    labelFa: "ارزان‌ترین",
    semantics: "Ascending effective price in selected currency.",
  ...
  },
  {
    value: "price-desc",
    labelFa: "گران‌ترین",
    semantics: "Descending effective price in selected currency.",
    requiredData: "comparable effective price",
    fallback: "newest",
    indexabilityEffect: "noindex-and-strip-from-canonical",
  },
  {
    value: "popular",
    labelFa: "محبوب‌ترین",
    semantics: "Descending documented popularity score.",
    requiredData: "analytics or order-derived popularity score with time window",
    fallback: "newest",
    indexabilityEffect: "noindex-and-strip-from-canonical",
  },
  {
    ...
    semantics: "Bayesian/threshold-adjusted rating; not raw average alone.",
    requiredData: "rating value and minimum rating count",
    fallback: "popular",
    indexabilityEffect: "noindex-and-strip-from-canonical",
  },
  {
    ...
    semantics: "Descending active discount percentage, then effective price.",
