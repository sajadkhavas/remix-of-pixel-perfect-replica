...
export class FixtureCatalogRepository implements CatalogRepository {
  async listProducts(query: CatalogQuery): Promise<PageResult<Product>> {
    ...
    if (query.brandSlugs?.length) {
      const ids = FIXTURE_BRANDS.filter((brand) => query.brandSlugs?.includes(brand.slug)).map(
        (brand) => brand.id,
      );
      items = items.filter((item) => ids.includes(item.brandId));
    }
    if (query.productIds?.length)
      items = items.filter((item) => query.productIds?.includes(item.identity.id));
...
export class FixtureProductRepository implements ProductRepository {
  async getById(id: EntityId) {
    return FIXTURE_PRODUCTS.find((product) => product.identity.id === id) ?? null;
  }
  async getBySlug(slug: Slug) {
    return FIXTURE_PRODUCTS.find((product) => product.identity.slug === slug) ?? null;
  }
  async listRelated(productId: EntityId, limit: number) {
    ...
    if (!source) return [];
    return FIXTURE_PRODUCTS.filter(
      (product) =>
        product.identity.id !== source.identity.id &&
        product.categoryIds.some((id) => source.categoryIds.includes(id)),
    ).slice(0, Math.max(0, limit));
  }
  async listReviews(_productId: EntityId, page: PageRequest): Promise<PageResult<Review>> {
...
export class FixtureBrandRepository implements BrandRepository {
  ...
  async getById(id: EntityId) {
    return FIXTURE_BRANDS.find((brand) => brand.id === id) ?? null;
  }
  async getBySlug(slug: Slug) {
    return FIXTURE_BRANDS.find((brand) => brand.slug === slug) ?? null;
  }
}
...
export class FixtureSearchRepository implements SearchRepository {
  async search(state: DiscoverySearchState): Promise<SearchResult> {
    ...
    switch (state.sort) {
      ...
      case "best-rated":
        sorted.sort(
          (a, b) => (b.reviewSummary?.ratingValue ?? 0) - (a.reviewSummary?.ratingValue ?? 0),
        );
        break;
      case "discount":
      ...
      case "popular":
        sorted.sort(
          (a, b) => (b.reviewSummary?.reviewCount ?? 0) - (a.reviewSummary?.reviewCount ?? 0),
        );
        break;
      case "relevance":
        break;
      ...
      facets: [
        {
          ...
          buckets: FIXTURE_BRANDS.map((brand) => ({
            valueKey: brand.slug,
            count: sorted.filter((product) => product.brandId === brand.id).length,
...
export class FixtureContentRepository implements ContentRepository {
  async listMagazine(page: PageRequest): Promise<PageResult<ContentSummary>> {
    return paginate(
      FIXTURE_CONTENT.filter((item) => item.type === "article"),
      page,
    );
  }
  async getArticleBySlug(slug: Slug) {
    return FIXTURE_CONTENT.find((item) => item.type === "article" && item.slug === slug) ?? null;
  }
  async getGuideBySlug(slug: Slug) {
    return FIXTURE_CONTENT.find((item) => item.type === "guide" && item.slug === slug) ?? null;
  }
}
