...
export function ProductCard({ watch }: { watch: Watch }) {
  ...
  return (
    ...
    >
      <Link to="/product/$id" params={{ id: String(watch.id) }} className="block relative overflow-hidden aspect-[3/4] bg-[#0D0D0D]">
        <motion.img
          src={watch.image}
          alt={watch.name}
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-1.5" dir="rtl">
          {watch.isNew && <span className="text-[9px] font-bold px-2 py-1 tracking-wider uppercase" style={{ background: accent, color: "#080808" }}>جدید</span>}
          {watch.isLimited && <span className="text-[9px] font-bold px-2 py-1 tracking-wider uppercase border" style={{ borderColor: accent, color: accent, background: `${accent}0d` }}>محدود</span>}
          {discount && <span className="text-[9px] font-bold px-2 py-1 bg-[#8B0000] text-white">-{discount}٪</span>}
        </div>
        <div className="absolute bottom-3 right-3 flex items-center gap-1.5">
          <div className={`w-1.5 h-1.5 rounded-full ${watch.stock === 0 ? "bg-red-600" : watch.stock < 3 ? "bg-amber-500 animate-pulse" : "bg-green-500"}`} />
          <span className="text-[10px] text-[#8A8A8A]">
            {watch.stock === 0 ? "ناموجود" : watch.stock < 3 ? "فقط " + watch.stock + " عدد" : "موجود"}
          </span>
        </div>
      </Link>
      ...
      <button
        ...
        onClick={(e) => { e.preventDefault(); toggleWishlist(watch.id); }}
        aria-label="افزودن به علاقه‌مندی"
      >
        <Heart className="w-4 h-4" style={{ color: wished ? "#C9A84C" : "#8A8A8A", fill: wished ? "#C9A84C" : "none" }} />
      </button>

      <div className="p-4 sm:p-5" dir="rtl">
        <span className="text-[10px] tracking-[0.25em] uppercase text-[#8A8A8A] block mb-1">{watch.brand}</span>
        ...
        <div className="flex gap-1.5 flex-wrap my-2.5">
          {watch.caseMaterial && <span className="text-[10px] text-[#8A8A8A] border border-[#2A2A2A] px-2 py-0.5">{watch.caseMaterial}</span>}
        ...
        {watch.rating && (
          <div className="flex items-center gap-1.5 mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} className="w-3 h-3" style={{ fill: i < Math.round(watch.rating!) ? "#C9A84C" : "transparent", color: "#C9A84C" }} />
            ))}
            {watch.review_count && <span className="text-[10px] text-[#8A8A8A]">({watch.review_count.toLocaleString("fa-IR")})</span>}
          </div>
        )}

        <div className="flex items-end justify-between mb-3">
          <div>
            <span className="text-lg sm:text-xl font-black block" style={{ color: accent, fontFamily: "DM Mono, monospace" }}>{displayPrice}</span>
            <span className="text-[10px] text-[#8A8A8A]">تومان</span>
          </div>
          {watch.sale_price && <span className="text-xs text-[#4A4A4A] line-through">{origPrice}</span>}
        </div>
        ...
        <motion.button
          ...
          style={{ background: watch.stock === 0 ? "#1E1E1E" : accent, color: watch.stock === 0 ? "#4A4A4A" : "#080808" }}
        >
