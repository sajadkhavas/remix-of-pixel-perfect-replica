...
function WishlistPage() {
  ...
  return (
    <>
      <PageHero eyebrow="علاقه‌مندی" title={items.length ? `${items.length.toLocaleString("fa-IR")} ساعت در لیست شما` : "لیست علاقه‌مندی خالی است"} />
      <section className="py-10 px-5 sm:px-8" dir="rtl">
        <div className="container mx-auto">
          {items.length === 0 ? (
            <div className="text-center py-12">
              <Heart className="w-16 h-16 text-[#2A2A2A] mx-auto mb-4" />
              <p className="text-[#8A8A8A] mb-6">روی آیکن قلب در کارت محصول کلیک کنید تا اینجا ذخیره شود.</p>
