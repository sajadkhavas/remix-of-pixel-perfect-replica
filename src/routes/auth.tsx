...
function AuthPage() {
  ...
  return (
    <>
      <PageHero eyebrow="حساب کاربری" title={mode === "login" ? "ورود به کرونوس" : "ساخت حساب کاربری"} />
        ...
        <div className="container mx-auto max-w-md">
          <div className="border border-[#1E1E1E] bg-[#0c0c0c] p-6">
              ...
              {(["login", "register"] as const).map((m) => (
                <button
                  ...
                  className={`flex-1 py-2.5 text-xs tracking-wider uppercase transition-colors ${
                    mode === m ? "bg-[#C9A84C] text-[#080808]" : "text-[#8A8A8A] hover:text-[#F0EDE8]"
                  }`}
                >
            ...
            >
              ...
              <button type="submit" className="w-full py-3 bg-[#C9A84C] text-[#080808] font-bold text-xs tracking-[0.2em] uppercase hover:bg-[#E8C96C]">
...
function Input({ label, type = "text" }: { label: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} required className="w-full bg-[#080808] border border-[#1E1E1E] focus:border-[#C9A84C55] outline-none px-3 py-2.5 text-sm text-[#F0EDE8]" />
    </div>
  );
}
