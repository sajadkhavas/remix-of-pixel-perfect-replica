...
function CartPage() {
  ...
  return (
    <>
      <PageHero
        eyebrow="سبد خرید"
        title={`${items.length.toLocaleString("fa-IR")} محصول در سبد شما`}
      />
      <section className="py-10 px-5 sm:px-8" dir="rtl">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-3">
            {items.map(({ watch, qty }) => (
              <div key={watch.id} className="border border-[#1E1E1E] bg-[#0c0c0c] p-4 flex gap-4">
                <Link
                  ...
                  className="w-20 sm:w-28 aspect-square bg-[#080808] shrink-0"
                >
                  <img
                    src={watch.image}
                    alt={watch.name}
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  <span className="text-[10px] tracking-wider text-[#8A8A8A] uppercase">
                    {watch.brand}
                  </span>
                  <Link
                    to="/product/$id"
                    params={{ id: String(watch.id) }}
                    className="font-bold text-[#F0EDE8] hover:text-[#C9A84C] truncate text-sm sm:text-base"
                  >
                    {watch.name}
                  </Link>
                  <span
                    className="text-[#C9A84C] font-black mt-1 text-sm sm:text-base"
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2">
                    <div className="flex items-center border border-[#1E1E1E]">
                      <button
                        onClick={() => setQty(watch.id, qty - 1)}
                        className="p-1.5 text-[#A8A8A8] hover:text-[#C9A84C]"
                        aria-label="کم"
                      >
                        <Minus className="w-3.5 h-3.5" />
                      </button>
                      <span className="px-3 text-sm text-[#F0EDE8]">
                    ...
                    <button
                      ...
                      className="text-[#8A8A8A] hover:text-red-500 p-1.5"
                      aria-label="حذف"
                    >
            ...
            <button
              onClick={clearCart}
              className="text-xs text-[#8A8A8A] hover:text-red-500 tracking-wider"
            >
              پاک کردن سبد
            </button>
          </div>
          ...
          <aside className="border border-[#1E1E1E] bg-[#0c0c0c] p-5 h-fit sticky top-24">
            ...
            <div className="flex items-end justify-between py-4">
              ...
              <span
                className="text-[#C9A84C] text-xl font-black"
                style={{ fontFamily: "DM Mono, monospace" }}
              >
            ...
            <Link
              to="/shop"
              className="block text-center mt-3 text-xs text-[#8A8A8A] hover:text-[#C9A84C]"
            >
              ادامه خرید
            </Link>
          </aside>
