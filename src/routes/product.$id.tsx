    ...
    <section className="py-8 sm:py-12 px-5 sm:px-8" dir="rtl">
      <div className="container mx-auto">
        <nav className="text-xs text-[#8A8A8A] mb-6 flex gap-1.5">
          ...
          <span>/</span>
          <Link to="/shop" className="hover:text-[#C9A84C]">
            فروشگاه
          </Link>
          <span>/</span>
          <Link
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          <div className="bg-[#0c0c0c] border border-[#1E1E1E] aspect-square flex items-center justify-center p-8 sm:p-16">
            <img
              ...
              loading="lazy"
              decoding="async"
              className="max-w-full max-h-full object-contain drop-shadow-[0_30px_50px_rgba(0,0,0,0.6)]"
            />
          </div>
          ...
          <div>
            ...
            <h1
              className="text-2xl sm:text-4xl font-black text-[#F0EDE8] mt-2 mb-4"
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      ...
                      className="w-4 h-4"
                      style={{
                        fill: i < Math.round(w.rating!) ? "#C9A84C" : "transparent",
                        color: "#C9A84C",
                      }}
                    />
                  ))}
                ...
                {w.review_count && (
                  <span className="text-xs text-[#8A8A8A]">
                    ({w.review_count.toLocaleString("fa-IR")} نظر)
                  </span>
                )}
              </div>
            )}
            ...
            <div className="flex items-end gap-3 mb-6 pb-6 border-b border-[#1A1A1A]">
              <span
                className="text-3xl sm:text-4xl font-black text-[#C9A84C]"
              ...
              {w.sale_price && (
                <span className="text-base text-[#4A4A4A] line-through mb-1 mr-auto">{orig}</span>
              )}
            </div>

            <p className="text-[#A8A8A8] leading-loose mb-6 text-sm sm:text-base">
              {w.brand} با مدل {w.name}، نمونه‌ای از طراحی دقیق و مهندسی ساعت‌سازی. مناسب برای
              علاقه‌مندان به ساعت‌های با کیفیت و طراحی منحصربه‌فرد.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 text-sm">
              ...
              <Spec
                k="وضعیت"
                v={w.stock === 0 ? "ناموجود" : w.stock < 3 ? `فقط ${w.stock} عدد` : "موجود"}
              />
            </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center border border-[#1E1E1E]">
                <button
                  ...
                  className="p-3 text-[#A8A8A8] hover:text-[#C9A84C]"
                  aria-label="کم"
                >
                  <Minus className="w-4 h-4" />
                </button>
                <span className="px-5 text-[#F0EDE8] min-w-[2.5rem] text-center">
              ...
              <button
                disabled={w.stock === 0}
                onClick={() => {
                  for (let i = 0; i < qty; i++) addToCart(w.id);
                }}
              ...
              <button
                ...
                className="p-3 sm:p-4 border border-[#2A2A2A] hover:border-[#C9A84C] transition-colors"
                aria-label="علاقه‌مندی"
              >
                <Heart
                  className="w-5 h-5"
                  style={{
                    color: wished ? "#C9A84C" : "#A8A8A8",
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-6 pt-6 border-t border-[#1A1A1A] text-xs">
              {[
                ...
                <div key={t} className="flex items-center gap-2 text-[#A8A8A8]">
                  <I className="w-4 h-4 text-[#C9A84C] shrink-0" />
                  <span>{t}</span>
                </div>
              ))}
            </div>
        ...
        {related.length > 0 && (
          <div className="mt-16 sm:mt-24">
            <h2
              className="text-2xl sm:text-3xl font-black text-[#F0EDE8] mb-6"
              style={{ fontFamily: "Playfair Display, Vazirmatn Variable, serif" }}
            >
              محصولات مشابه
            </h2>
