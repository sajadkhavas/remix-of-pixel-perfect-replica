...
function BrandsPage() {
  return (
    <>
      <PageHero eyebrow="برندها" title="معتبرترین خانه‌های ساعت‌سازی" sub="از ژنو تا توکیو، انتخاب شما از میان برترین برندهای دنیا." />
      <section className="py-10 px-5 sm:px-8" dir="rtl">
        <div className="container mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {BRANDS.map((b) => (
            ...
            >
              <span className="text-xl sm:text-2xl font-black text-[#F0EDE8] group-hover:text-[#C9A84C] transition-colors tracking-[0.1em]" style={{ fontFamily: "Playfair Display, serif" }}>
                {b.name}
              </span>
              <span className="text-[10px] sm:text-xs text-[#8A8A8A] mt-2 tracking-wider">{b.tagline}</span>
            </Link>
          ))}
        </div>
      </section>
